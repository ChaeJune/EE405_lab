...
class MotionPlanner
{
  public:
    ...
    double CalculateCollisionCost(const std::vector<Node>& motionPrimitive);
    double CalculateSteeringCost(const std::vector<Node>& motionPrimitive);
    double CalculateProgressCost(const std::vector<Node>& motionPrimitive);
    double CalculateDistanceToObstacle(const Node& motionNode, const nav_msgs::OccupancyGrid& localMap);
    double AccumulateOfDistance(double distance);
    double AccumulateOfSteering(double deltaSteering);
    double CalculateDistanceBetweenNodes(const Node& node1, const Node& node2);
    double AccumulateOfProgress(double distanceCovered);

    // Parameters
    double FOV = 85.2 * (M_PI / 180.0); // [rad] FOV of point cloud (realsense)
    double MAX_SENSOR_RANGE = 10.00; // [m] maximum sensor range (realsense)
    ...
    double MAX_DELTA = 10.00 * (M_PI / 180.0); // [rad] maximum angle for steering angle sampling
    double MAX_PROGRESS = 5.0; // [m] max progress of motion
    double INFLATION_SIZE = 0.55 / DIST_RESOL; // [grid] inflation size [m] / grid_res [m/grid]
    ...
    double WCOLLISION = 0.2;
    double WSTEERING = 0.4;
    double WPROGRESS = 0.4;
  ...
  private:
    ...
    ros::Publisher pubCommandTh;
    ros::Publisher pubCommandSt;
    ros::Publisher pubCommandMo;
